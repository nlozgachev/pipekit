...
export namespace Reader {
   ...
   * ```
   */
  export const map = <R, A, B>(f: (a: A) => B) => (data: Reader<R, A>): Reader<R, B> => (env) =>
    f(data(env));
   ...
   */
  export const chain =
    <R, A, B>(f: (a: A) => Reader<R, B>) => (data: Reader<R, A>): Reader<R, B> => (env) =>
      f(data(env))(env);
   ...
   */
  export const ap =
    <R, A>(arg: Reader<R, A>) => <B>(data: Reader<R, (a: A) => B>): Reader<R, B> => (env) =>
      data(env)(arg(env));
   ...
   */
  export const local =
    <R2, R>(f: (env: R2) => R) => <A>(data: Reader<R, A>): Reader<R2, A> => (env) => data(f(env));
